...
#define VPACK(version, state) (((version) << 3) | ((state) & 7))
...
static void init_probe_bound(PHTable *t, uint64_t h) { STORE(&t->bounds[h & t->mask], BPACK(0, false), RELAXED); }
static uint64_t get_probe_bound(PHTable *t, uint64_t h) {
    uint64_t b = LOAD(&t->bounds[h & t->mask], ACQUIRE);
    return GET_BOUND(b);
}
static void cond_raise_bound(PHTable *t, uint64_t h, uint64_t idx) {
    ...
    bool scanning;
    do {
        b = LOAD(&t->bounds[h & t->mask], ACQUIRE);
        obound = GET_BOUND(b);
        scanning = b & 1;
        nb = BPACK(MAX(obound, idx), false);
    } while (!CMPXCHG(&t->bounds[h & t->mask], &b, nb, ACQ_REL, RELAXED));
}
...
static void cond_lower_bound(PHTable *t, uint64_t h, uint64_t idx) {
    ...
    if (scanning) {
        ...
        CMPXCHG(&t->bounds[h & t->mask], &expect, BPACK(bound, false), ACQ_REL, RELAXED);
    }
    if (idx) {
        expect = BPACK(idx, false);
        if (CMPXCHG(&t->bounds[h & t->mask], &expect, BPACK(idx, true), ACQ_REL, RELAXED)) {
            uint64_t i = idx - 1;
            while (i > 0 && !has_collision(t, h, i)) {
                i--;
            }
            expect = BPACK(idx, true);
            CMPXCHG(&t->bounds[h & t->mask], &expect, BPACK(i, false), ACQ_REL, RELAXED);
        }
    }
}
static bool assist(PHTable *t, BNode *k, uint64_t h, uint64_t i, uint64_t ver_i, bool (*eq)(BNode *, BNode *)) {
    ...
    for (int j = 0; j <= max; j++) {
        if (i != j) {
            ...
            uint32_t state_j = vsj & 7;
            if (state_j == B_INSERTING && eq(LOAD(&buc_j->node, RELAXED), k)) {
                if (j < i) {
            ...
            ver_j = GET_VER(vsj);
            state_j = vsj & 7;
            if (state_j == B_MEMBER && eq(LOAD(&buc_j->node, RELAXED), k)) {
                uint64_t expect = VPACK(ver_i, B_INSERTING);
                CMPXCHG(&buc_i->vs, &expect, VPACK(ver_i, B_COLLIDED), ACQ_REL, RELAXED);
                return false;
            }
