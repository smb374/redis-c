...
// FNV-hash 64-bit
uint64_t int_hash(const uint64_t val) {
...
uint64_t bytes_hash(const uint8_t* bytes, const size_t len) {
...
uint64_t vstr_hash(const vstr* s) {
    uint32_t h = 0x811C9DC5;
    for (size_t i = 0; i < s->len; i++) {
        h = (h + s->dat[i]) * 0x01000193;
    }
    return h;
}
