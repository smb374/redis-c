...
// --- Test Fixture ---
class CHPMapTest : public ::testing::Test {
protected:
    ...
    void SetUp() override {
        qsbr_init(65536);
        qsbr_reg();
        cmap = chpm_new(nullptr, MAP_SIZE);
...
TEST_F(CHPMapTest, SingleThreadInsertContainsRemove) {
    ...
    // Add
    ASSERT_TRUE(chpm_add(cmap, &entry1->node, test_entry_eq));
    ASSERT_TRUE(chpm_add(cmap, &entry2->node, test_entry_eq));
    ASSERT_EQ(chpm_size(cmap), 2);

    // Add duplicate
    ASSERT_FALSE(chpm_add(cmap, &entry1->node, test_entry_eq));
    ASSERT_EQ(chpm_size(cmap), 2);

    // Contains
    ASSERT_TRUE(chpm_contains(cmap, &query1.node, test_entry_eq));

    // Remove
    ASSERT_TRUE(chpm_remove(cmap, &query1.node, test_entry_eq));
    ASSERT_EQ(chpm_size(cmap), 1);
    ASSERT_FALSE(chpm_contains(cmap, &query1.node, test_entry_eq));

    // Remove non-existent
    ASSERT_FALSE(chpm_remove(cmap, &query1.node, test_entry_eq));
...
TEST_F(CHPMapTest, MultiThreadAllNodesPresent) {
    ...
    for (size_t i = 0; i < NUM_THREADS; ++i) {
        threads.emplace_back([&, thread_id = i]() {
            ...
            for (uint64_t k = start_key; k < end_key; ++k) {
                ...
                all_entries[k] = entry;
                ASSERT_TRUE(chpm_add(cmap, &entry->node, test_entry_eq));
            }
            qsbr_quiescent();
            qsbr_unreg();
        });
    ...
    ASSERT_EQ(chpm_size(cmap), NUM_THREADS * keys_per_thread);
    for (uint64_t k = 0; k < NUM_THREADS * keys_per_thread; ++k) {
        TestEntry query{{int_hash_rapid(k)}, k, 0};
        ASSERT_TRUE(chpm_contains(cmap, &query.node, test_entry_eq)) << "Key " << k << " was not found.";
    }
    qsbr_quiescent();
...
TEST_F(CHPMapTest, MultiThreadMixedReadWrite) {
    ...
    for (int i = 0; i < key_space; ++i) {
        ...
        chpm_add(cmap, &entry->node, test_entry_eq);
    }
...
TEST_F(CHPMapTest, SingleThreadUpsert) {
    auto *entry1 = new TestEntry{{int_hash_rapid(100)}, 100, 1000};
    auto *entry2_new = new TestEntry{{int_hash_rapid(100)}, 100, 2000}; // Same key, new value

    // 1. Upsert a new key
    BNode *result_node = chpm_upsert(cmap, &entry1->node, test_entry_eq);
    ASSERT_EQ(result_node, &entry1->node); // Should return the new node
    ASSERT_EQ(chpm_size(cmap), 1);

    // Verify the content
    BNode *found_node = chpm_contains(cmap, &entry1->node, test_entry_eq)
                                ? &container_of(chpm_upsert(cmap, &entry1->node, test_entry_eq), TestEntry, node)->node
                                : nullptr;
    ...
    result_node = chpm_upsert(cmap, &entry2_new->node, test_entry_eq);
    ASSERT_EQ(result_node, &entry1->node); // Should return the ORIGINAL node
    ASSERT_EQ(chpm_size(cmap), 1); // Size should not change

    // 3. Verify the value has not changed
    found_node = chpm_contains(cmap, &entry1->node, test_entry_eq)
                         ? &container_of(chpm_upsert(cmap, &entry1->node, test_entry_eq), TestEntry, node)->node
                         : nullptr;
    ASSERT_NE(found_node, nullptr);
